...
namespace MegaMini
{
    internal class MegaAesCtrStreamDecrypter : MegaAesCtrStream
    {
        ...
        public MegaAesCtrStreamDecrypter(Stream stream, long streamLength, byte[] fileKey, byte[] iv, byte[] expectedMetaMac) : base(stream, streamLength, Mode.Decrypt, fileKey, iv)
        {
    ...
    internal abstract class MegaAesCtrStream : Stream
    {
        ...
        protected MegaAesCtrStream(Stream stream, long streamLength, Mode mode, byte[] fileKey, byte[] iv)
        {
            ...
            _encryptor = Utils.CreateAesEncryptor(FileKey);
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            for (var pos = _position; pos < Math.Min(_position + count, StreamLength); pos += 16)
            {
                ...
                var encryptedIvCounter = Utils.EncryptAes(ivCounter, _encryptor);
                ...
                _currentChunkMac = Utils.EncryptAes(_currentChunkMac, _encryptor);
            }
        ...
        private void ComputeChunk(ICryptoTransform encryptor)
        {
            ...
            _fileMac = Utils.EncryptAes(_fileMac, encryptor);
        }
